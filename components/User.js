...
const User = ({ item }) => {
    ...
    const fetchFriendRequests = async () => {
      try {
        const response = await fetch(
          `http://172.20.101.191:3000/friend-requests/sent/${userId}`
        );
  ...
  useEffect(() => {
    const fetchUserFriends = async () => {
      try {
        const response = await fetch(`http://172.20.101.191:3000/friends/${userId}`);
  ...
  const sendFriendRequest = async (currentUserId, selectedUserId) => {
    try {
      const response = await fetch("http://172.20.101.191:3000/friend-request", {
        method: "POST",
        headers: {
